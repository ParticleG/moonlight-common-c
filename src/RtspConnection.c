...
static int initializeRtspRequest(PRTSP_MESSAGE msg, char *command, char *target) {
    ...
    if (!addOption(msg, "CSeq", sequenceNumberStr) || !addOption(msg, "X-GS-ClientVersion", clientVersionStr) || (!useEnet && !addOption(msg, "Host", urlAddr))) {
        freeMessage(msg);
        return 0;
    }
...
static int transactRtspMessageEnet(PRTSP_MESSAGE request, PRTSP_MESSAGE response, int expectingPayload, int *error) {
    ...
    *error         = -1;
    ...
    if (serializedMessage == NULL) { goto Exit; }
    ...
    // Wait for a reply
    if (serviceEnetHost(client, &event, RTSP_TIMEOUT_SEC * 1000) <= 0 || event.type != ENET_EVENT_TYPE_RECEIVE) {
...
Exit:
    ...
    if (serializedMessage != NULL) { free(serializedMessage); }
...
static int transactRtspMessageTcp(PRTSP_MESSAGE request, PRTSP_MESSAGE response, int expectingPayload, int *error) {
    ...
    *error         = -1;
    ...
    sock = connectTcpSocket(&RemoteAddr, RemoteAddrLen, ServerInfo.portBoth0, RTSP_TIMEOUT_SEC);
...
static int requestDescribe(PRTSP_MESSAGE response, int *error) {
    ...
    *error = -1;
    ...
    if (ret != 0) {
        if (addOption(&request, "Accept", "application/sdp") && addOption(&request, "If-Modified-Since", "Thu, 01 Jan 1970 00:00:00 GMT")) {
...
static int setupStream(PRTSP_MESSAGE response, char *target, int *error) {
    ...
    *error = -1;
    ...
    if (ret != 0) {
        ...
        if (addOption(&request, "Transport", transportValue) && addOption(&request, "If-Modified-Since", "Thu, 01 Jan 1970 00:00:00 GMT")) {
            ret = transactRtspMessage(&request, response, 0, error);
        } else {
            ret = 0;
        }
...
static int sendVideoAnnounce(PRTSP_MESSAGE response, int *error) {
    ...
    *error = -1;
    ...
    if (ret != 0) {
        ...
        if (request.payload == NULL) { goto FreeMessage; }
        request.flags |= FLAG_ALLOCATED_PAYLOAD;
        request.payloadLength = payloadLength;

        sprintf(payloadLengthStr, "%d", payloadLength);
        if (!addOption(&request, "Content-length", payloadLengthStr)) { goto FreeMessage; }
...
int performRtspHandshake(void) {
    ...
    if (OriginalVideoBitrate >= HIGH_AUDIO_BITRATE_THRESHOLD && (AudioCallbacks.capabilities & CAPABILITY_SLOW_OPUS_DECODER) == 0) {
    ...
    char tempStr[64] = "rtsp%s://%s:", tempPort[6];
    itoa(ServerInfo.portBoth0, tempPort, 10);
    strcat(tempStr, tempPort);
    sprintf(rtspTargetUrl, tempStr, useEnet ? "ru" : "", urlAddr);

    currentSeqNumber = 1;
    hasSessionId     = 0;

    switch (AppVersionQuad[0]) {
        case 3: rtspClientVersion = 10; break;
        case 4: rtspClientVersion = 11; break;
        case 5: rtspClientVersion = 12; break;
        case 6:
            // Gen 6 has never been seen in the wild
            rtspClientVersion = 13;
            break;
        case 7:
        default: rtspClientVersion = 14; break;
    }
    ...
    if (useEnet) {
        ...
        enet_address_set_port(&address, ServerInfo.portBoth0);
        ...
        if (serviceEnetHost(client, &event, RTSP_TIMEOUT_SEC * 1000) <= 0 || event.type != ENET_EVENT_TYPE_CONNECT) {
            char logStr[64] = "RTSP: Failed to connect to UDP port ", logPort[6];
            itoa(ServerInfo.portBoth0, logPort, 10);
...

    {
        ...
        if (StreamConfig.supportsHevc && strstr(response.payload, "sprop-parameter-sets=AAAAAU")) {
            ...
            if (StreamConfig.width > 4096 || StreamConfig.height > 4096) { Limelog("WARNING: Host PC doesn't support HEVC. Streaming at resolutions above 4K using H.264 will likely fail!\n"); }
        }
